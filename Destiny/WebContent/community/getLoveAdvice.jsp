...
<script type="text/javascript">
	...
	$(function(){
		...
		$(".modify").on("click", function() {
			...
			self.location = "/love/updateLoveAdvice?communityNo=${community.communityNo}"
		});
		/* 수정 버튼 이벤트 : end */
		...
		$(".delete").on("click", function() {
			//self.location = "/info/updateRestaurantInfo?communityNo="+${community.communityNo}
			self.location = "/love/deleteLoveAdvice?communityNo=${community.communityNo}"
		});
		/* 수정 버튼 이벤트 : end */
		...
		/* 공감 버튼 이벤트 : start */
			...
			$.ajax({
				method : "POST",
				url : '/love/json/likeLoveAdvice/'+communityNo,
				headers : {
		...
		$("button:contains('이전글')").on("click", function(){
			...
			self.location="/love/getPreLoveAdvice?communityNo="+communityNo
		});
		/* 이전글 클릭 : end */
		...
		$("button:contains('다음글')").on("click", function(){
			var communityNo = $(this).data("param")
			self.location="/love/getNextLoveAdvice?communityNo="+communityNo
		});
		/* 다음글 클릭 : end */
	});
